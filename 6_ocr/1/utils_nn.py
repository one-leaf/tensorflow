...
def resNet50v2(layer, isPoolSize=True, stride=2):
    if isPoolSize:
        stride = stride
        padding = "SAME"
    else:
...
def resNextBlockB(inputs, depth=4):
    layers_split = []
    ...
    layer = slim.conv2d(layer, size, [1,1], normalizer_fn=slim.batch_norm, activation_fn=None)
    return tf.nn.leaky_relu(inputs + layer)   
