...
def imwrite(imageName,image):
    cv2.imwrite(os.path.join(curr_dir,"test",imageName),image)
...
def preprocess(gray):
    ...
    # 3. 膨胀和腐蚀操作的核函数
    element1 = cv2.getStructuringElement(cv2.MORPH_RECT, (20, 5))
    element2 = cv2.getStructuringElement(cv2.MORPH_RECT, (21, 8))
...
if __name__ == '__main__':
    # 读取文件
    imagePath = os.path.join(curr_dir,"test","1.jpg")
    img = cv2.imread(imagePath)
    detect(img)

# 这种办法不行，只能用fastrcnn来分拣区域