...
def neural_networks():
    ...
    d_optim = tf.train.AdamOptimizer(LEARNING_RATE_INITIAL*0.01).minimize(d_loss, global_step=global_step, var_list=d_vars)
        ...
        while True:
            for batch in range(BATCHES):
                ...
                if errA > errD2 or (errD2 < 0.5 and errA > 0.5):
                    ...
                    print("%d time: %4.4fs, g_loss: %.8f (mse: %.6f adv: %.6f), d_loss2: %.6f" % (steps, time.time() - start, errG, errM, errA, errD2))
