...
def resNetBlockV3(inputs, size=64):
    layer0 = slim.conv2d(inputs, size//4,   [1,1], normalizer_fn=slim.batch_norm, activation_fn=tf.nn.relu)
    layer1 = slim.conv2d(inputs, size,   [1,1], normalizer_fn=slim.batch_norm, activation_fn=tf.nn.relu)
    layer1 = slim.conv2d(layer1, size//4,   [3,3], normalizer_fn=slim.batch_norm, activation_fn=tf.nn.relu)
    layer2 = slim.conv2d(inputs, size,   [1,1], normalizer_fn=slim.batch_norm, activation_fn=tf.nn.relu)
    layer2 = slim.conv2d(layer2, size//4,   [5,5], normalizer_fn=slim.batch_norm, activation_fn=tf.nn.relu) 
    layer3 = slim.avg_pool2d(inputs, [3, 3], stride = 1, padding = "SAME")  
    layer3 = slim.conv2d(layer3, size//4,   [1,1], normalizer_fn=slim.batch_norm, activation_fn=tf.nn.relu) 
