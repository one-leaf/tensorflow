...
def neural_networks():
    ...
    d_loss1 = 1e-5 * tf.losses.sigmoid_cross_entropy(logits_real, tf.ones_like(logits_real))
    d_loss2 = -1e-5 * tf.losses.sigmoid_cross_entropy(logits_fake, tf.ones_like(logits_fake))
    d_loss  = d_loss1 + d_loss2

    g_gan_loss = 1e-5 * tf.losses.sigmoid_cross_entropy(logits_fake, tf.ones_like(logits_fake))
    ...
    with tf.Session() as session:
        ...
        while True:
            for batch in range(BATCHES):
                ...
                if steps > 0 and steps % REPORT_STEPS < 21:
