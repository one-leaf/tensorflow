...
def LSTM(inputs, fc_size, lstm_size):
    ...
    for i in range(3):
        with tf.variable_scope("rnn-%s"%i):
            ...
            # activation 用 tanh 根本学习不出来 , elu 在后期有更好的表现
            cell_fw = tf.contrib.rnn.GRUCell(lstm_size, activation=tf.nn.sigmoid)
            cell_bw = tf.contrib.rnn.GRUCell(lstm_size, activation=tf.nn.sigmoid)
...
def get_next_batch_for_res(batch_size=128, _font_name=None, _font_size=None, _font_mode=None, _font_hint=None):
    ...
    max_width_image = 0
    info = []
    font_length = random.randint(5, 100)
