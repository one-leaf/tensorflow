...
def neural_networks():
    ...
    grads, norm = tf.clip_by_global_norm(tf.gradients(res_loss, res_vars), 10.0)
    res_optim = res_optim.apply_gradients(list(zip(grads, res_vars)), global_step=global_step)
...
def get_next_batch_for_res(batch_size=128, _font_name=None, _font_size=None, _font_mode=None, _font_hint=None):
    ...
    max_width_image = 0
    info = []
    font_length = random.randint(5, 350)
