...
def neural_networks_trim():
    ...
    layer = slim.fully_connected(inputs, 64)
    layer = slim.fully_connected(inputs, 128)
    layer = slim.fully_connected(inputs, 256)
    layer = slim.fully_connected(inputs, 512)
    layer = slim.fully_connected(inputs, 256)
    layer = slim.fully_connected(inputs, 128)
    layer = slim.fully_connected(inputs, 64)
    layer = slim.fully_connected(layer, 1, activation_fn= tf.nn.tanh)
    print(layer.shape)
