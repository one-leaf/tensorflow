...
def neural_networks_trim():
    ...
    layer = slim.conv2d(layer,64,[1,1])
    layer = slim.fully_connected(layer,64)
    layer = slim.conv2d(layer,128,[1,1])
    layer = slim.fully_connected(layer,128)
    layer = slim.conv2d(layer,256,[1,1])
    layer = slim.fully_connected(layer,256)
    layer = slim.conv2d(layer,512,[1,1])
    layer = slim.fully_connected(layer,512)
    layer = slim.conv2d(layer,256,[1,1])
    layer = slim.fully_connected(layer,256)
    layer = slim.conv2d(layer,128,[1,1])
    layer = slim.fully_connected(layer,128)
    layer = slim.conv2d(layer,64,[1,1])
    layer = slim.fully_connected(layer,64)
    layer = slim.conv2d(layer,1,[1,1], activation_fn=None)
