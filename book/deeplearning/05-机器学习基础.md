...
1. 贝叶斯统计
    ...
    1. 用最大似然解：
        ...
        $$log(L)=mlog(\theta)+(n-m)log(1-\theta)$$ 
        
        求导：

        $$\frac {\partial log(L)}{\partial \theta}=\frac {m}{\theta}-\frac {n-m}{1-\theta}=0$$

        解：

        $$\frac {m}{\theta}=\frac {n-m}{1-\theta}$$

        $$\Rightarrow\theta=\frac {m}{n}$$
    ...
    2. 用贝叶斯解
        ...
        $P(\theta|data)$ 就是需要求的后验概率

        假设一共投掷了n次，其中m次正面朝上：

        $$P(\theta|data)=\frac {P(data|\theta)P(\theta)}{P(data)}=\frac {\theta^m*\theta}{\theta^m*\theta+(1-\theta)^{n-m}*(1-\theta)}$$

        简化

        $$P(\theta|data)=\frac {\theta^{m+1}}{\theta^{m+1}+(1-\theta)^{n-m+1}}$$


        $$=\frac {1}{1+\frac {(1-\theta)^{n-m+1}}{\theta^{m+1}}}$$
...
1. 各损失函数的总结
    ...
    - 均方误差
        ...
        $$p(x|\mu,\sigma^2)=\prod N(x|\mu,\sigma^2)=\prod \frac {1}{\sigma\sqrt {2\pi}}\exp(-\frac {(x_i-\mu)^2}{2\sigma^2})$$
