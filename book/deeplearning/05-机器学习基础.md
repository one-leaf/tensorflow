...
1. 最大似然估计
    ...
    例子：考虑投掷一枚硬币的实验，硬币的两面有可能不均，需要按观察到的结果估算合理性。

    1. 先假设这枚硬币是公平的，投掷正面的概率$P(H)=0.5$

    2. 如果投掷了两面朝上，一面朝下，用似然表示为：

        $$L=P(H)*P(H)*(1-P(H))=0.5*0.5*(1-0.5)=0.125$$

    3. 如果$P(H)=0.6$，则重新计算似然为：

        $$L=P(H)*P(H)*(1-P(H))=0.6*0.6*(1-0.6)=0.144$$

    4. 可以发现当$P(H)=0.6$时，似然函数L的值比当$P(H)=0.5$时的值大，因此，$P(H)=0.6$更可靠

    5. 通过求导$\frac {d\theta^2(1-\theta)}{d\theta}=0$计算，找到当L最大值时$P(H)$的值为$P(H)=\frac 23$，因此得出当 $P(H)=\frac 23$ 时是最合理的。
    ...
    $$\hat\theta_{ML}=arg\max_\theta \log p(X;\theta)$$

    为了方便计算，除以n取平均值，转为:

    $$\hat\theta_{ML}=arg\max_\theta \mathbb E_x\log p(X;\theta)$$
    ...
    $$L(\theta)=\ln\prod ^n_{i=1}p(x_i;\theta)$$

    $$\frac {\partial L(\theta)}{\partial\theta}
    =\sum_{i=1}^n\frac{\partial\ln p(x_i;\theta)}{\partial\theta}
    ...
    $$\ln L(\theta)=\ln(p(x_1,x_2;\theta))=\sum ^{10}_{i=1}\ln p(x_i;\theta)$$

    两边取对数后，左边称为对数似然。

    平均对数似然：

    $$\hat e=\frac 1{10}lnL(\theta)$$
    ...
    $$\hat e=\frac 1{10}lnL(\theta)=\frac 1{10}P(x_1,x_2,...,x_{10}|M)=\frac 1{10}ln(\theta^7(1-\theta)^3)$$
    ...
    $$\hat e'(θ)=\sum_{i=1}^n\frac{1}{p(x_i;\theta)}\frac{\partial p(x_i;\theta)}{\partial\theta}
...
1. 监督学习算法
    ...
    - 无监督学习算法
        ...
        - 主成分分析
            ...
            PCA使用线性投影，将输入x映射为z，使用最大方差的方向和新空间的轴对齐。
            ...
            4. 求cov(c)的特征值和特征向量
                ...
                4. 取最大的$\lambda$带入多项式得到特征向量

                    $$\begin{bmatrix}16.91-11.67&-7.33\\-7.33&16.91-6.67\end{bmatrix}x=\begin{bmatrix}5.24&-7.33\\-7.33&10.24\end{bmatrix}x=0$$
