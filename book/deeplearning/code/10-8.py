#https://github.com/francesco-mannella/Echo-State-Networks/blob/master/ESN.py
...
class network():
    ...
    def wr_initizer(self, shape, partition_info=None):
        wr = tf.random_normal(shape)
        connectivity_mask = tf.cast(tf.less_equal(tf.random_uniform(shape), 0.1), dtype=tf.float32)
        print(wr.get_shape(), connectivity_mask.get_shape())
        wr = tf.multiply(wr, connectivity_mask)
        wr_norm2 = tf.sqrt(tf.reduce_sum(tf.square(wr)))
        is_norm_0 = tf.cast(tf.equal(wr_norm2,0), dtype=tf.float32)
        wr = wr * 0.7 / (wr_norm2 + 1 * is_norm_0)
        return wr

    def add_rnn_layer(self, inputs, batch_size, cell_num, ):
        input_size = inputs.get_shape().as_list()[-1]
        win = tf.Variable(tf.random_normal([input_size, cell_num]), trainable=False)
        wr  = tf.Variable(self.wr_initizer([cell_num, cell_num]), trainable=False)
        b   = tf.Variable(tf.random_normal([cell_num]), trainable=False)
        ...
        def compute(i, cur_state, out):
            in_mat = tf.concat([inputs[i], cur_state], axis=1)
            weights_mat = tf.concat([win, wr], axis=0)
            output = tf.nn.tanh(tf.matmul(in_mat, weights_mat)+ b)
            return i+1, output, out.write(i, output)
