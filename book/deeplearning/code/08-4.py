    ...
    def __init__(self):
        ...
        for i, width in enumerate(layer_widths):
            if width == layer_widths[-1]:
                ...
                layer = self.add_layer(self.teacher_layers[-1], width, True, False, 'teacher_layer_%s'%i)
        ...
        for i, width in enumerate(layer_widths):
            with tf.variable_scope('student_network'):
                if width==layer_widths[-1]:
                    ...
                    layer = self.add_layer(self.student_layers[-1], width, i!=5, i!=5, 'student_layer_%s'%i)
