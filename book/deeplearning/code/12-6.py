...
class translate_network():
    ...
    def get_rnn_cell(self, rnn_size, rnn_layers_num):
        def single_cell():
            ...
            return cell
        return tf.contrib.rnn.MultiRNNCell([single_cell() for _ in range(rnn_layers_num)])

    def __init__(self, en_words, zh_words, embedding_size, rnn_size=128, rnn_layers_num=3, beam_search=True, beam_size=10):
        ...
        print(self.en_embedding)
        print(self.zh_embedding)

        # 单词转向量
        self.en_embedded = tf.nn.embedding_lookup(self.en_embedding, self.en)
        self.zh_embedded = tf.nn.embedding_lookup(self.zh_embedding, self.zh)

        # encoder 编码器
        with tf.variable_scope('encoder'):
            fw_cell = self.get_rnn_cell(rnn_size, rnn_layers_num)
            bw_cell = self.get_rnn_cell(rnn_size, rnn_layers_num)
            bi_outputs, bi_state = tf.nn.bidirectional_dynamic_rnn(fw_cell, bw_cell, self.en_embedding, self.en_seq_len, dtype=tf.float32)
        ...
        with tf.variable_scope('decoder'):
            ...
            if self.beam_search:
                ...
                memory = tf.contrib.seq2seq.tile_batch(memory, self.beam_size)
                x_len = tf.contrib.seq2seq.tile_batch(self.en_seq_len, self.beam_size)
            ...
            # attention = tf.contrib.seq2seq.BahdanauAttention(rnn_size, memory, x_len, normalize=True)
            ...
            decoder_initial_state = decoder_cell.zero_state(bs, tf.float32).clone(cell_state=encoder_state)
            ...
            train_helper = tf.contrib.seq2seq.TrainingHelper(self.zh_embedding, self.zh_seq_len)
        ...
        with tf.variable_scope("decoder", reuse=True):
            ...
            if self.beam_search:
                infer_decoder = tf.contrib.seq2seq.BeamSearchDecoder(decoder_cell, self.zh_embedding, 
                ...
                infer_helper = tf.contrib.seq2seq.GreedyEmbeddingHelper(self.zh_embedding, start_tokens, KEY_WORDS["<EOS>"])   
            ...
            max_zh_seq_len = tf.reduce_max(self.zh_seq_len)
            mask = tf.sequence_mask(self.zh_seq_len, max_zh_seq_len, dtype=tf.float32)
            logits_train = tf.arg_max(self.logits, axis=-1)
            self.cost = tf.contrib.seq2seq.sequence_loss(logits=logits_train, targets=self.zh_embedding, weights=mask)
...
def main():
    ...
    test_eng_sentence = "Knowledge is power."
    ...
    net =  translate_network(en_words_dict, zh_words_dict, embedding_size, beam_search=True)

    # 训练SEQ-SEQ网络
    train(en_sentences_vec, zh_sentences_vec, net, batch_size=64)
