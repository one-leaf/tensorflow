...
class network():
    ...
    def __init__(self):
        ...
        layer1 = self.add_conv_layer(x_image, 7, 8, activation_function=tf.nn.relu, pool_function=tf.nn.max_pool) 
        layer1 = self.add_conv_layer(layer1, 3, 16, activation_function=tf.nn.relu, pool_function=tf.nn.max_pool) 
        layer1 = self.add_conv_layer(layer1, 3, 32, activation_function=tf.nn.relu, pool_function=tf.nn.max_pool) 

        layer2 = self.add_conv_layer(x_image, 5, 8, activation_function=tf.nn.relu, pool_function=tf.nn.max_pool) 
        layer2 = self.add_conv_layer(layer2, 5, 16, activation_function=tf.nn.relu, pool_function=tf.nn.max_pool) 
        layer2 = self.add_conv_layer(layer2, 3, 32, activation_function=tf.nn.relu, pool_function=tf.nn.max_pool) 

        layer3 = self.add_conv_layer(x_image, 3, 8, activation_function=tf.nn.relu, pool_function=tf.nn.max_pool) 
        layer3 = self.add_conv_layer(layer3, 3, 16, activation_function=tf.nn.relu, pool_function=tf.nn.max_pool) 
        layer3 = self.add_conv_layer(layer3, 3, 32, activation_function=tf.nn.relu, pool_function=tf.nn.max_pool) 

        layer = tf.concat([layer1,layer2,layer3], -1)
