...
- 行列式
    ...
    行列式，det(A) ,是将方阵A映射到实数的函数，行列式等于矩阵特征值的乘积。一个矩阵的行列式就是一个平行多面体的体积，这个多面体的每条边对应着矩阵的列。
...
- 实用举例
    ...
    扁平化X 得 X‘ = [1,28*28] = [1, 784] 的矩阵

    $$X'=\begin{bmatrix} x_{(1)}\\ \vdots \\ x_{(784)} \end{bmatrix} $$

    定义参数 w 为 [10, 28*28] = [10, 784] 的矩阵， 10为手写体数字[0~9]，共10个分类。

    $$w=\begin{bmatrix} w_{(1,1)}&...&w_{(1,10)}\\ \vdots &\vdots &\vdots \\ w_{(784,1)}&...&w_{(10,784)} \end{bmatrix}$$

    定义偏置 b 为 [1, 10] = [1, 10] 的矩阵

    $$b=\begin{bmatrix} b_{(1)}\\ \vdots \\b_{(10)}\end{bmatrix}$$

    根据线性方程组得输出y，手写体数字[0~9]的每一个数字的概率：

    $$y=X'*w+b=\begin{bmatrix} x_{(1)}\\ \vdots \\ x_{(784)} \end{bmatrix}  * \begin{bmatrix} w_{(1,1)}&...&w_{(1,10)}\\ \vdots &\vdots &\vdots \\ w_{(784,1)}&...&w_{(10,784)} \end{bmatrix} + \begin{bmatrix} b_{(1)}\\ \vdots \\b_{(10)}\end{bmatrix}=\begin{bmatrix} y_{(1)}\\ \vdots \\y_{(10)}\end{bmatrix}$$
