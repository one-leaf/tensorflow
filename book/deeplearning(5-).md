5. 机器学习基础
    ...
    2. 容量、过拟合和欠拟合
        ...
        例子，某种特征，得癌症的测出为阳性的几率为90%，未得癌症测出的几率为90%，而人群中得癌症的几率为1%。问一个人被测出阳性，得癌症的几率是多少？

        中间有个问题是采用的样本分布不均，所以不能直接得出是90%，需要考虑1%的样本分布。

        我们用A表示测试出阳性，Y表示得癌症，N表示未得癌症，根据如上得：

        $$P(A|Y)=0.9,P(A|N)=0.1,P(Y)=0.01,P(N)=0.99$$

        那么，已知为阳性的情况，得癌症概率P(Y,A)为联合概率，等于人群中得癌症的概率乘以得癌症时为阳性的概率:

        $$P(Y,A)=P(Y)*P(A|Y)=0.01*0.9=0.009$$

        同理，未得癌症，但检测出来为阳性的概率为：

        $$P(N,A)=P(N)*P(N|Y)=0.99*0.1=0.099$$
