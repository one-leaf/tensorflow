...
def add_zero_data_to_list(label, v_data):
    ...
    for i in range(0, w-block_size, 2):
        if max(label[i:i+block_size]) == 0 or min(label[i:i+block_size]) == 1:
...
def pre_data():
    ...
    for c, data in enumerate(training_data):
        ...
        # print("readed %s/%s %s.pkl, size: %s/%s/%s"%(c,size,data["id"],len(data_2),len(data_1),len(data_0[0])))
    ...
    def reader():
        ...
        while len(data_0[0])>1000 or t1.isAlive(): 
            ...
            if _size >500 and (_size > buf_size or must_pop):
                datas.append(_data.pop(0))
            else:
                datas.append(random.choice(_data))
