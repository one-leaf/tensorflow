...
def network():
    ...
    main_nets = []
    net = cnn2(x,  block_size+1,  1, 64, 1, block_size//2)
    net = cnn2(net, block_size//2+1, 64, 64, 1, block_size//2//2)
    net = cnn2(net, block_size//4+1, 64, 64, 1, block_size//4//2)
    net = cnn2(net, block_size//8+1, 64, 64, 1, block_size//8//2)
    net = cnn2(net, block_size//16+1, 64, 64, 1, block_size//16//2)
    net = cnn2(net, block_size//32+1, 64, 64, 1, block_size//32//2)
    net = cnn2(net, block_size//32+1, 64, 64, 1, block_size//32//2)
...
def readDatatoPool():
    ...
    for i in range(size):
        ...
        for i in range(w):
            ...
            if i!=w-1 and (i< train_size or random.random() > 1./64): continue
            fix_segments =[]
