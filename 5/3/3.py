...
INITIAL_RANDOM_ACTION_PROB = 0.80  # 随机移动的最大概率 0.90
...
# CNN网络
def get_network(x, output_size, filter_size=[8,8,5,5,3,3,3,3,3,3], filter_nums=[128,128,128,128,128,128,128,128,128,128], pool_scale=[2,2,2,2,2,2,2,2,2,2], pool_type=[0,0,0,1,0,1,0,1,0,1],full_nums=1024, output_name="output_layer"):
...
def train():    
    ...
    learning_rate = tf.train.exponential_decay(1e-6, global_step, 1000000, 0.98, staircase=True)
    # 学习函数
    ...
    while True:
        ...
        if reward != 0.0:  
            ...
            if not _game_random_step:
                ...
                print(shape,reward,'*') 
            if not terminal: # 如果是最后一步，就不计算下一步了
        ...
        if len(_observations) > MEMORY_SIZE:
            ...
            if _step % SAVE_EVERY_X_STEPS == 0:
                ...
                print("step: %s random_prob: %s scores: %s reward: %s" %
                  (_step, _probability_of_random_action, sum(_last_scores) / STORE_SCORES_LEN, reward))
