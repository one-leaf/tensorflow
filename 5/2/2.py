...
def convolutional_neural_network(input_image):
    ...
    if DEBUG:
        ...
        filter_map1 = tf.transpose(conv1[-1], perm=[2, 0, 1])   
        filter_map1 = tf.reshape(filter_map1, (32, int(filter_map1.get_shape()[1]), int(filter_map1.get_shape()[2]), 1)) 
        tf.summary.image('conv1', tensor=filter_map1,  max_outputs=32)      
        filter_map2 = tf.transpose(conv2[-1], perm=[2, 0, 1])   
        filter_map2 = tf.reshape(filter_map2, (32, int(filter_map2.get_shape()[1]), int(filter_map2.get_shape()[2]), 1)) 
        tf.summary.image('conv2', tensor=filter_map2,  max_outputs=32) 
    ...
    with tf.Session() as sess:
        ...
        while not coord.should_stop():            
            ...
            if D_size >= BATCH:
                ...
                _step,_train_summary_op,_,_cost,_y =sess.run([global_step,train_summary_op,optimizer,cost,predict_action],feed_dict = {gt : gt_batch, argmax : argmax_batch, input_image : input_image_data_batch})
