			...
			for event in pygame.event.get():
				if event.type == QUIT:
					...
					sys.exit()
                        ...
                        (event.type == pygame.KEYDOWN and event.key == pygame.K_LEFT):  # 鼠标左键按下(左移)
					bar_move_left = True
				elif (event.type == pygame.MOUSEBUTTONUP and event.button == 1) or \
                        (event.type == pygame.KEYUP and event.key == pygame.K_LEFT) : # 鼠标左键释放
					bar_move_left = False
				elif (event.type == pygame.MOUSEBUTTONDOWN and event.button == 3) or \
                        (event.type == pygame.KEYDOWN and event.key == pygame.K_RIGHT): #右键
					bar_move_right = True
