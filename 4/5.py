# coding=utf-8
# 卷积多层多项验证码识别
# lable 改为了 onehot 并且验证码图片学习随机从 fonts 目录抽取

import numpy as np
from utils import img2gray, img2vec, text2vec, vec2text, loadimg
...
y_ = tf.placeholder(tf.int32, [None, captcha_size * char_size], name='y_')
...
for i in range(captcha_size):
    with tf.variable_scope('loss-part-{}'.format(i)):
        ...
        # loss_part = tf.nn.softmax_cross_entropy_with_logits(logits=outputs_part, labels=targets_part) #需要 onehot 化才可以
        # loss_part = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=outputs_part, labels=targets_part) # 不能 onehot
